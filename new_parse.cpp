   ...
   while (iter != end) {
     ...
     switch (comma_cnt) {
       ...
       default:
           ...
           *enclosing_quotes = ++iter == '"';

           if (*enclosing_quotes) { // Check for enclosing quotes.
          ...
          auto temp_str = std::move( line.substr(start_offset, length - enclosing_quotes ? 1 : 0) ); 
